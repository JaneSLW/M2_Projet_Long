...
cd ${WDIR}
# dos2unix ${REFNAME}.bed

# divise le fichier bed en fonction de la longuer de l'exon
awk '{if ( ($3-$2+1) <=25) {printf("%s\n",$0);}}' ${BED} > BED_600.bed # si l'exon a moins de 25 bases alors on attend au moin 60% de match
awk '{if ((($3-$2+1) <=50) && (($3-$2+1)>25)) {printf("%s\n",$0);}}' ${BED} > BED_185.bed # 18,5%
...
bedtools intersect -f 0.${L} -wo -a BED_${L}.bed -b ../${REFNAME}  # filtre les reads avec le nb de match minimum
done > ${FNAME}.txt
